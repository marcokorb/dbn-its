...
class NetworkSubject(models.Model):
    """NetworkSubject model."""

    network = models.ForeignKey(
        Network, on_delete=models.CASCADE, related_name="subjects"
    )

    subject = models.ForeignKey(
        Subject, on_delete=models.CASCADE, related_name="networks_subjects"
    )

    network_subjects = models.ManyToManyField(
        Subject, blank=True, related_name="networks_subjects_parents"
    )

    evidences = models.ManyToManyField(
        Evidence, related_name="networks_subjects_evidences"
    )
...
class NetworkSubjectProbabilities(models.Model):
    """NetworkSubjectProbabilities model."""

    network_subject = models.ForeignKey(
        NetworkSubject, on_delete=models.CASCADE, related_name="probabilities"
    )
