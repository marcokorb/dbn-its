...
def user_question(request):
    """Get user current question."""
    course = Course.objects.get(
        id=request.query_params["course_id"]
    )  # pylint: disable=maybe-no-member
    ...
    if not UserCourseQuestion.objects.filter(
        course=course, user=user
    ).exists():  # pylint: disable=maybe-no-member
        ...
        last_user_question = UserCourseQuestion.objects.filter(
            course=course, user=user
        ).latest(
            "pk"
        )  # pylint: disable=maybe-no-member
        ...
        course_question = (
            CourseQuestion.objects.filter(
                course=course,
                number__gt=last_user_question.course_question.number,
            )
            .order_by("number")
            .first()
        )  # pylint: disable=maybe-no-member
...
def user_answer(request):
    ...
    alternative: Alternative = Alternative.objects.get(
        id=alternative_id, question_id=question_id
    )  # pylint: disable=maybe-no-member
    ...
    course_question: CourseQuestion = CourseQuestion.objects.get(
    ...
    question_evidences: List[int] = question.evidences.values_list(
        "pk", flat=True
    ).distinct()

    # Retrieve user course question
    (
        user_course_question,
        _,
    ) = UserCourseQuestion.objects.get_or_create(  # pylint: disable=maybe-no-member
        course=course,
        course_question=course_question,
        network=course.network,
        user=user,
    ...
    for evidence in course_evidences:
        ...
        observation = (
            int(alternative.status)
            if evidence.pk in question_evidences
            else " "
        )
    ...
    return Response({"alternative_status": alternative.status})
